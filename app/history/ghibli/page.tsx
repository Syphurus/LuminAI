...
export default function ImageHistoryPage() {
    ...
    const fetchImages = async () => {
      try {
        const res = await fetch(
          `${BACKEND_URL}/api/ghibli-image-history?user_id=${user.id}`
        );
  ...
  const handleDelete = async (id: string) => {
    try {
      await fetch(`${BACKEND_URL}/api/ghibli-image-history/${id}`, {
        method: "DELETE",
      });
  ...
  return (
    ...
    >
      <h1 className="text-3xl font-bold">🖼️ Ghibli History</h1>
      {images.length > 0 ? (
